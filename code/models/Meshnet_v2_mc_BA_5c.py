...
print("MeshNet five-channel with BA regions and corpus callosum using v2 started")
...
def extract_cc_from_aseg(segmentation_file, label_ids, apply_affine=True):
    ...
    apply_affine (bool): Whether to apply affine transformation to the vertices.
...
def create_dataloader(
    ...
    ba_labels_channel_3=['BA1', 'BA2'],  # Added BA1 and BA2 channel
    ba_labels_channel_4=['BA3a', 'BA3b'],  # Added BA3a and BA3b channel
    ...
    ba_labels_channel_3 (list): List of BA labels for channel 3. (New)
    ba_labels_channel_4 (list): List of BA labels for channel 4. (New)
    ...
    for (surf_folder, label_folder, mri_folder), label in zip(data, labels):
        ...
        # Process BA regions (channels 1 to 4)
        hemispheres = ['lh', 'rh']
        for ba_labels in [ba_labels_channel_1, ba_labels_channel_2, ba_labels_channel_3, ba_labels_channel_4]:
            all_normals = []
            all_corners = []
            all_centers = []
        ...
        # Process Corpus Callosum (channel 5)
        ...
        # Check if data is available for all regions (4 BA channels + CC)
        if len(region_normals) != 5:
...
class MeshNetSeparatePaths(nn.Module):
    def __init__(self, num_classes=2, num_regions=5, dropout_rate=0.3):  # Updated num_regions to 5
...
def plot_training_curves(metrics_dict, hyperparams):
    ...
    plt.savefig(f"./models/meshnet_v2_mc_BA_5c/training_curves/training_curves_lr{hyperparams[0]}_dropout{hyperparams[1]}_batch_size_{hyperparams[2]}.png")
    plt.show()
...
def plot_confusion_matrix(conf_mt_counter, y_true, y_pred, classes):
    ...
    plt.tight_layout()
    plt.savefig(f'./models/meshnet_v2_mc_BA_5c/confusion_matrix/confusion_matrix_{conf_mt_counter}.png')
    plt.show()
...
if __name__ == '__main__':
    ...
    os.makedirs('./models/meshnet_v2_mc_BA_5c/confusion_matrix', exist_ok=True)
    os.makedirs('./models/meshnet_v2_mc_BA_5c/hyperparameters', exist_ok=True)
    os.makedirs('./models/meshnet_v2_mc_BA_5c/training_curves', exist_ok=True)
    ...
    learning_rates = [0.0001, 0.00001, 0.000001] # best 0.000001
    dropout_rates = [0, 0.3, 0.5] # best 0.3
    batch_sizes = [64, 128]  # best 128
    ...
    for lr, dropout, batch_size in product(learning_rates, dropout_rates, batch_sizes):
        ...
        # Create DataLoaders with multiple regions (5 regions now)
        train_loader = create_dataloader(
            train_data, train_labels, batch_size=batch_size, augment=False,
            target_faces=target_faces,
            ba_labels_channel_1=['BA4a', 'BA4p'],
            ba_labels_channel_2=['BA6'],
            ba_labels_channel_3=['BA1', 'BA2'],  # New channel
            ba_labels_channel_4=['BA3a', 'BA3b'],  # New channel
        ...
        model = MeshNetSeparatePaths(num_classes=2, num_regions=5, dropout_rate=dropout).to(device)  # Updated num_regions to 5
        ...
        if metrics_dict['val_acc'][-1] > best_val_acc:
            ...
            torch.save(model.state_dict(), './models/meshnet_v2_mc_BA_5c/best_model.pth')
    ...
    plt.legend()
    plt.savefig('./models/meshnet_v2_mc_BA_5c/hyperparameters/search_results.png')
    plt.show()
    ...
    torch.save(all_metrics, './models/meshnet_v2_mc_BA_5c/hyperparameters/all_metrics.pt')
