...
print("MeshNet multi channel Started")
...
class EnhancedMeshNet(nn.Module):
    ...
    def forward(self, data):
        ...
        num_faces = data.num_faces // (self.num_regions * batch_size)  # Assuming uniform meshes
...
def plot_training_curves(metrics_dict, hyperparams):
    ...
    plt.savefig(f"./models/meshnet_mc_2/training_curves/training_curves_lr{hyperparams[0]}_dropout{hyperparams[1]}_batch_size_{hyperparams[2]}.png")
    plt.show()
...
def plot_confusion_matrix(conf_mt_counter, y_true, y_pred, classes):
    ...
    plt.tight_layout()
    plt.savefig(f'./models/meshnet_mc_2/confusion_matrix/confusion_matrix_{conf_mt_counter}.png')
    plt.show()
...
if __name__ == '__main__':
    ...
    os.makedirs('./models/meshnet_mc_2/confusion_matrix', exist_ok=True)
    os.makedirs('./models/meshnet_mc_2/hyperparameters', exist_ok=True)
    os.makedirs('./models/meshnet_mc_2/training_curves', exist_ok=True)
    ...
    batch_sizes = [16,32,64,128]  # Set batch_size to 32
    ...
    for lr, dropout, batch_size in product(learning_rates, dropout_rates, batch_sizes):
        ...
        if metrics_dict['val_acc'][-1] > best_val_acc:
            ...
            torch.save(model.state_dict(), './models/meshnet_mc_2/best_model.pth')
    ...
    plt.legend()
    plt.savefig('./models/meshnet_mc_2/hyperparameters/search_results.png')
    plt.show()
    ...
    torch.save(all_metrics, './models/meshnet_mc_2/hyperparameters/all_metrics.pt')
