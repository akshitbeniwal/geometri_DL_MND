...
print("MeshNet with Separate Convolutional Paths Started")
...
def extract_regions_from_pial(pial_file, annot_file, target_region_names):
    ...
    for target_region_name in target_region_names:
        if target_region_name in names:
            ...
            target_label = ctab[target_label_index, -1]
        else:
            print(f"Region {target_region_name} not found")
...
def create_dataloader(data, labels, batch_size=32, augment=False, target_faces=1024, target_region_names=['precentral', 'paracentral']):
    dataset = []
    num_regions = len(target_region_names)

    for (surf_folder, label_folder), label in zip(data, labels):
        # Process both hemispheres
        hemispheres = ['lh', 'rh']
        region_normals = [[] for _ in range(num_regions)]
        region_corners = [[] for _ in range(num_regions)]
        region_centers = [[] for _ in range(num_regions)]
        ...
        for hemi in hemispheres:
            ...
            if os.path.exists(pial_file) and os.path.exists(annot_file):
                ...
                for region_idx, (vertices, faces) in enumerate(extracted_regions):
                    ...
                    region_normals[region_idx].append(normals)
                    region_corners[region_idx].append(corners)
                    region_centers[region_idx].append(centers)
            else:
                print(f'Missing pial or annot file for {surf_folder}')

        # Check if data is available for all regions
        if any(len(r) == 0 for r in region_normals):
            continue  # Skip this subject if data is missing for any region

        # Concatenate data from all hemispheres per region
        all_region_normals = []
        all_region_corners = []
        all_region_centers = []
        desired_faces = target_faces * len(hemispheres)  # Adjusted per region

        for r_normals, r_corners, r_centers in zip(region_normals, region_corners, region_centers):
            normals = torch.cat(r_normals, dim=0)
            corners = torch.cat(r_corners, dim=0)
            centers = torch.cat(r_centers, dim=0)

            # Enforce uniform number of faces per region
            current_faces = normals.shape[0]
            ...
            all_region_normals.append(normals)
            all_region_corners.append(corners)
            all_region_centers.append(centers)

        # Create Data object
        data_obj = Data(
            normals=all_region_normals,   # List of tensors per region
            corners=all_region_corners,
            centers=all_region_centers,
...
# MeshNet model with separate convolutional paths for each region
class MeshNetSeparatePaths(nn.Module):
    def __init__(self, num_classes=2, num_regions=2):
    ...
    def forward(self, data):
        ...
        for i in range(self.num_regions):
            ...
            x = F.relu(self.conv1[i](combined_features))
            x = F.relu(self.conv2[i](x))
            x = F.relu(self.conv3[i](x))
...
def plot_training_curves(metrics_dict, hyperparams):
    ...
    plt.savefig(f"./models/meshnet_separate_paths/training_curves/training_curves_lr{hyperparams[0]}_dropout{hyperparams[1]}_batch_size_{hyperparams[2]}.png")
    plt.show()
...
def plot_confusion_matrix(conf_mt_counter, y_true, y_pred, classes):
    ...
    plt.tight_layout()
    plt.savefig(f'./models/meshnet_separate_paths/confusion_matrix/confusion_matrix_{conf_mt_counter}.png')
    plt.show()
...
if __name__ == '__main__':
    ...
    os.makedirs('./models/meshnet_separate_paths/confusion_matrix', exist_ok=True)
    os.makedirs('./models/meshnet_separate_paths/hyperparameters', exist_ok=True)
    os.makedirs('./models/meshnet_separate_paths/training_curves', exist_ok=True)
    ...
    for lr, dropout, batch_size in product(learning_rates, dropout_rates, batch_sizes):
        ...
        train_loader = create_dataloader(
            ...
            target_faces=target_faces, target_region_names=['precentral', 'paracentral']
        )
        ...
        model = MeshNetSeparatePaths(num_classes=2, num_regions=2).to(device)
        ...
        if metrics_dict['val_acc'][-1] > best_val_acc:
            ...
            torch.save(model.state_dict(), './models/meshnet_separate_paths/best_model.pth')
    ...
    plt.legend()
    plt.savefig('./models/meshnet_separate_paths/hyperparameters/search_results.png')
    plt.show()
    ...
    torch.save(all_metrics, './models/meshnet_separate_paths/hyperparameters/all_metrics.pt')
